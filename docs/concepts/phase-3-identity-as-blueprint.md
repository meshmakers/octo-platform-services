...
| #26 | octo-common-services | `a30c7de` | Follow-up #2: `System.Notification.Bootstrap-1.0.0` + CK type move System.Identity → System.Notification (§6) |
| #91 | octo-identity-services | `d1950b3` | Follow-up #2: delete `CreateTenantConfiguration`, `IsServiceManagedBlueprint` covers `System.Notification.`, migration 18→19 `ckTypeId` rewrite |
| #92 | octo-identity-services | `2d26cd1` | Follow-up #2 cleanup: migration 19→20 deletes pre-blueprint `MailNotificationConfiguration` + `NotificationTemplate` duplicates |
...
- **`System.Notification.Bootstrap-1.0.0`** for the mail templates + `RtMailNotificationConfiguration` Identity used to seed inline. **Status: ✅ Shipped 2026-06-15.** Three PRs landed back-to-back:
  - **PR #26** `octo-common-services` `a30c7de` — bumped `System.Notification` CK model 2.0.0 → 2.1.0 to claim `MailNotificationConfiguration` (the type + `EnableEmailNotifications` / `RedirectAfterEmailInteractionUrl` attributes were always notification-config; the YAML just lived in `System.Identity-2.7.0` because the imperative seed did). Ships `System.Notification.Bootstrap-1.0.0` (4 entities at stable rtIds `680…01`, `680…10..12`) and the generated `AddBlueprintSystemNotificationBootstrapV1()` registration in `AddOctoNotification()`.
  - **PR #91** `octo-identity-services` `d1950b3` — dropped the CK type from `System.Identity-2.7.0 → 2.8.0`, rewired 3 .cs consumers to the Notifications-generated namespace, replaced `ServiceManagedBlueprintPrefix` with an `IsServiceManagedBlueprint` override that matches both `System.Identity.` and `System.Notification.`, deleted `CreateTenantConfiguration` (-72 LOC), registered self-referential `${Username}` / `${IdentityServerUrl}` / `${ConfirmToken}` placeholders in `IdentityBlueprintVariableProvider` so mail-body interpolation is silent. Migration 18 → 19 (`MailNotificationConfigurationCkTypeIdMigration`) rewrites the `ckTypeId` field on existing tenant docs from `System.Identity/MailNotificationConfiguration` to `System.Notification/MailNotificationConfiguration` using `GetRtEntitiesByTypeForMigrationAsync` + `UpdateCkTypeIdForMigrationAsync` (works even after the source type is gone from the CK cache; same collection, only the discriminator flips).
  - **PR #92** `octo-identity-services` `2d26cd1` — burn-in surface: the imperative seed's pre-blueprint rows (random rtIds) survived alongside the new blueprint's stable-rtId entities, so every tenant ended up with duplicates. Migration 19 → 20 (`PreBlueprintNotificationCleanupMigration`) mirrors `PreBlueprintCleanupMigration`'s contract: walks every `RtMailNotificationConfiguration` + `RtNotificationTemplate` and deletes anything outside the `680…00..680…FF` stable range.

  Net effect: Phase 3 closes with **zero** imperative seed code in `DefaultConfigurationCreatorService` (1063 → 296 LOC end-to-end across Phase 3 + #2). Every entity Identity provisions is now declarative seed-data in a service-managed blueprint.
...
3. **Inline `CreateTenantConfiguration` (Notification templates)** → **decided: leave for Phase 3.** Lifted out as Phase 3 follow-up #2 (`System.Notification.Bootstrap-1.0.0`, see §6). Shipped 2026-06-15 in PRs #26 / #91 / #92.
...
Concept shipped 2026-06-15; follow-up #2 `System.Notification.Bootstrap-1.0.0` shipped same day. The remaining natural items are the **octo-communication-sdk consumer-PRs** (8 repos pending the 3.4.6 → 3.4.x bump for the SDK split — see [[octo_communication_sdk_phase35]]) and the **drift-dashboard UI** on top of the live Phase 2 observability API, scoped against the cascading `System.UI.SystemCockpit` (octosystem) vs `System.UI.TenantCockpit` (child) Meshboard split.
