...
**Tracking:** `AB#4209`.
**Scope:** `octo-identity-services` (CK model + pre-cleanup gate), `octo-tools` (overlay file + cmdlet), `octo-cli` (standalone overlay apply/clean commands). Builds directly on Phase 3 (`phase-3-identity-as-blueprint.md`); does **not** introduce a new template engine — that already shipped 2026-06-15.

> **Decision update (2026-07-01, AB#4209):** The overlay apply is a **standalone, explicitly-invoked command** — Start-Octo does **not** auto-run it (the original §4.4 / Step 4 auto-apply idea was dropped). Likewise the template-clean export is delivered as its own `CleanClientOverlays` command run *before* `DumpTenant`, **not** as a `DumpTenant --clean` flag (original §4.7 / Step 5). Both changes follow the same principle: keep overlay operations as separate, opt-in commands rather than magic embedded in another command's lifecycle. The §4.4, §4.7, §5, and §7 #6 text below is retained for history but is superseded by this note.
...
3. A new `Apply-IdentityOverlay` cmdlet applies the overlay idempotently. It is invoked **explicitly by the developer** — Start-Octo does not auto-run it (decision 2026-07-01; see the superseding note at the top).
...
5. A standalone `octo-cli CleanClientOverlays` command strips `overlay:*` entries before a template-clean `DumpTenant` export (decision 2026-07-01; replaces the earlier `DumpTenant --clean` flag idea). `base` and `api` entries survive.
...
| 3 | octo-tools | Add `Apply-IdentityOverlay` cmdlet + `overlays/identity-local-dev.yaml`. Standalone, explicitly invoked. Also ships the server-side `ApplyClientOverlay` REST endpoint + `octo-cli ApplyClientOverlay` command. | low |
| 4 | ~~octo-tools~~ | ~~Wire `Start-Octo` to auto-invoke `Apply-IdentityOverlay`~~ **DROPPED 2026-07-01.** Overlay apply stays a standalone, explicitly-invoked command; no Start-Octo lifecycle hook, no `-SkipOverlay` flag. | — |
| 5 | octo-cli / octo-identity-services | Ship a standalone `CleanClientOverlays` command (+ server-side `CleanOverlayEntries` endpoint) that strips `overlay:*` entries before a template-clean `DumpTenant`. Replaces the earlier `DumpTenant --clean` flag idea. **Follow-up: mirror `ApplyClientOverlay` + `CleanClientOverlays` into the MCP server and the Refinery Studio client-admin UI (AB#4209).** | low — additive standalone command |
